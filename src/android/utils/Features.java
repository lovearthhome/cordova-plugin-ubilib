...
public class Features{
	...
	public static double variance(double data[]){
		...
		for (int i = 0; i<length; i++)
		{
			s = s + Math.pow(data[i] - average, 2);// 偏离平均数的距离和 
		}
		s = s / length;//方差 
		return s;
	}
	...
	/**四分位数（Quartile） 3/4**/
	public static double doublehirdQuartile(double data[]){
		int length = data.length;
		double[] copydata = new double[length];
		for (int i = 0; i < length; i++)
		{
	...
	/**四分位数（Quartile） 3/4**/
	public static double doublehirdQuartile(float data[]){
		int length = data.length;
		double[] copydata = new double[length];
		for (int i = 0; i < length; i++)
		{
	...
	/**四分位数（Quartile） 3/4**/
	public static double doublehirdQuartile(int data[]){
		int length = data.length;
		double[] copydata = new double[length];
		for (int i = 0; i < length; i++)
		{
	...
	/**求数组过零率**/
	public static double zeroCrossingRate(int data[]){
		int length = data.length;
		double num = 0;
		for (int i = 0; i < length - 1; i++)
		{
	...
	/**相关系数*/
	public static double correlation(int data1[], int data2[]){
		int length1 = data1.length;
		int length2 = data2.length;
		double mean1 = mean(data1);
		double mean2 = mean(data2);
